...
```mermaid
flowchart LR
    A[Event evidence] --> D[One connected example]
    B[Public market research] --> D
    C[One reviewed profile] --> D
    D --> E[Materials and example workflow]
    E --> F[Polished local demonstration]
    F --> G[Selective live-use decision]
```
